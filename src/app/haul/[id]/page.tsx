...
export default async function HaulDetailPage({ params }: PageProps<"/haul/[id]">) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="mb-4 text-2xl font-bold">{haul.name || "Untitled haul"}</h1>
        ...
        <div className="card grid grid-cols-1 gap-3 p-4 sm:grid-cols-2">
          ...
          <Field label="Temp Location Notes" value={haul.tempLocationNotes || "—"} />
          <Field
            label="Logged on"
      ...
      <div>
        <h2 className="mb-3 text-lg font-bold">
          Other Haul on {formatDateShort(haul.date)}
        </h2>
